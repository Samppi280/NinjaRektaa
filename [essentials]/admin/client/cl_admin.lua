	...
	while true do
		...
		if wp then
			...
			if zHeigt == 0.0 then
				...
				height = 1000.0
				zHeigt = 0.0
				drawNotification("~g~Menit markeriin!")
				break
			end
		end
	end
...
function admin_no_clip()
	...
	if noclip then -- activé
		...
		Notify("Noclip ~g~päälle")
	else -- désactivé
		SetEntityVisible(ped, true, false)
		Notify("Noclip ~r~pois")
	end
end
...
Citizen.CreateThread(function()
	...
	WarMenu.SetSubTitle('MainMenu', 'Vicerp Admin menu ')
...
-- Other	
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			DrawScaleformMovie(scaleform, 1.183, 0.6247, 0.9, 0.9, 255, 255, 255, 255, 0)
			if WarMenu.MenuButton('Admin menu ~b~>~s~', 'SelfMenu') then
			elseif WarMenu.MenuButton('Autot ~b~>~s~', 'VehMenu') then
			elseif WarMenu.MenuButton('Pelaajaan kohdistuvat ~b~>~s~', 'PlayerMenu') then
		end
...
-- Self Menu			
		...
		if WarMenu.CheckBox('Poista npc lähistöltä', ClearPNJ, function(enabled)
			ClearPNJ = enabled
		end) then
		elseif WarMenu.CheckBox("Näkymätömyys",Invisible,function(enabled)
			Invisible = enabled
		end) then
		elseif WarMenu.Button('Lennä') then
			admin_no_clip()
		elseif WarMenu.Button('NOCLIP nopeus + ') then
			noclip_speed = noclip_speed + 0.3
		elseif WarMenu.Button('No Clip nopeus - ') then
			noclip_speed = noclip_speed - 0.3
		elseif WarMenu.Button("~h~~r~Poista auto") then
			--DelVeh(GetVehiclePedIsUsing(PlayerPedId()))       EN DEV
			TriggerEvent('esx:deleteVehicle')
			drawNotification("Auto poistettu")
		elseif WarMenu.Button('Healaa') then
			SetEntityHealth(PlayerPedId(), 200)
			TriggerServerEvent("logMenuAdmin", '\n- Se redonne de la vie ( Heal )')
		elseif WarMenu.Button('Anna armoria') then 
			SetPedArmour(PlayerPedId(), 200)
		elseif WarMenu.Button("Anna ammuksia") then
			for i = 1, #allWeapons do
			AddAmmoToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 200)
		end
			TriggerServerEvent("logMenuAdmin", '\n- Se redonne de l\'armure( Heal )')
		elseif WarMenu.CheckBox('Kuolematomuus', Godmode, function(enabled)
			Godmode = enabled
		end) then
		elseif WarMenu.Button('Mene markeriin') then
			TeleportToWaypoint()
		--elseif WarMenu.Button("Refresh les ventes de véhicule") then
			TriggerEvent('esx-qalle-sellvehicles:refreshVehicles')
		elseif WarMenu.Button("Mene markeriin auton kanssa") then
		...
		if (NearestVehicle == 0) and (NearestPlane == 0) then
			drawNotification("poista autosta")
		...
		elseif WarMenu.Button('Tapaittes') then
			SetEntityHealth(PlayerPedId(), 0)
			drawNotification("~r~You Committed Suicide.")
		elseif WarMenu.Button('vaihda auton rekarin ') then
...
-- Véhicule Menu
		...
		elseif WarMenu.IsMenuOpened('VehMenu') then
			if WarMenu.MenuButton('Spawnaa ajoneuvo ~o~>~s~', 'SpawnVehMenu') then
			elseif WarMenu.Button("Vaihda rekisterikilpi") then
				local playerPed = GetPlayerPed(-1)
				local playerVeh = GetVehiclePedIsIn(playerPed, true)
				local result = KeyboardInput("Aseta haluamasi rekisterikilpi", "", 10)
				if result then
					SetVehicleNumberPlateText(playerVeh, result)
				end
			elseif WarMenu.Button('Korjaa ajoneuvo') then
				SetVehicleFixed(GetVehiclePedIsUsing(PlayerPedId()))
				TriggerServerEvent("logMenuAdmin", 'Ajoneuvon korjaus')
			elseif WarMenu.Button("Pese ajoneuvo") then
				Clean2(GetVehiclePedIsUsing(PlayerPedId()))
			elseif WarMenu.Button("Poista ajoneuvo") then
				TriggerEvent('esx:deleteVehicle')
				TriggerServerEvent("logMenuAdmin", 'Poistettu ajoneuvo')
			elseif WarMenu.CheckBox('Poista ajoneuvoja läheltäsi', deleteVeh, function(enabled)
					deleteVeh = enabled
				end) then
		...
		elseif WarMenu.IsMenuOpened('PlayerMenu') then
			if WarMenu.CheckBox('Näytä pelaajien blipit', blips1, function(enabled)
					blips1 = enabled
			...
			elseif WarMenu.MenuButton("Pelaajat ~b~>~s~", "OnlinePlayerMenu") then
			end
		...
		elseif WarMenu.IsMenuOpened('kickall') then
			...
			elseif WarMenu.Button('~g~Nosta kaikki') then
...
-- Menu autre joueur
			...
			DrawScaleformMovie(scaleform, 1.183, 0.6247, 0.9, 0.9, 255, 255, 255, 255, 0)
		elseif WarMenu.IsMenuOpened('PlayerMenu') then
			if WarMenu.CheckBox('Näytä pelaajian blipit', blips1, function(enabled)
					blips1 = enabled
				end) then
			elseif WarMenu.MenuButton("Pelaajat ~b~>~s~", "OnlinePlayerMenu") then
			end
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			if WarMenu.Button('Spectaapelaajaa', (Spectating and "~g~[SPECTATAAN]")) then
				SpectatePlayer(SelectedPlayer)
				name = GetPlayerName(SelectedPlayer)
				TriggerServerEvent("logMenuAdmin", '\nSpec un joueur ('..name..')')
			elseif WarMenu.Button('Teleportaa pelaajaan') then
				local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
				SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)
			elseif WarMenu.MenuButton('Anna Ase', 'SingleWepPlayer') then
			elseif WarMenu.Button('Anna auto') then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Auton spawni nimi", "", 100)
			...
			elseif WarMenu.Button("Nosta") then
				TriggerServerEvent('esx_ambulancejob_awe:revive', GetPlayerServerId(SelectedPlayer))
			end
